...
def draw_trajectory(queue, output, init_pose, cam_scale,
        ...
        while True:
            try:
                ...
                if data[0] == 'pose':
                    ...
                    draw_trajectory.mesh = draw_trajectory.mesh.crop(
                        o3d.geometry.AxisAlignedBoundingBox(min_bound=np.asarray([-0.5, -0.5, -0.005]),
                                                            max_bound=np.asarray([0.5, 0.5, 0.25])))
                    draw_trajectory.mesh = draw_trajectory.mesh.filter_smooth_simple(number_of_iterations=5)
    ...
    vis.create_window(window_name=output, height=480, width=640)
    ...
    ## Zooming level
    init_pose[:3, 3] += 1.5 * normalize(init_pose[:3, 2])
    init_pose[:3, 2] *= -1
    init_pose[:3, 1] *= -1
